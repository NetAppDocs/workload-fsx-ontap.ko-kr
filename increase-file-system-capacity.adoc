...
---
= NetApp Workload Factory에서 파일 시스템 용량을 수동으로 조정합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
사용량이 지정된 임계값을 초과하면 SSD 스토리지 용량을 늘리십시오. 작업 세트를 사용하지 않을 때는 SSD 스토리지 용량을 줄여 비용을 절감하십시오.

또는 link:manage-capacity-automatically.html["자동 용량 관리 활성화"] Workload Factory가 파일 시스템 용량을 관리하도록 설정할 수도 있습니다.
