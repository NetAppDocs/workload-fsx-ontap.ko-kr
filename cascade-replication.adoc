...
* 스토리지 시스템 간의 안전한 데이터 전송 및 관리를 위해 TCP 포트 11104 및 11105를 열어 두십시오. ICMP는 선택 사항이지만 네트워크 점검 및 문제 해결에 도움이 될 수 있습니다.
...
NOTE: 장기 보존을 위해서는 소스 레이블과 대상 레이블이 일치해야 합니다. 필요한 경우 Workload Factory에서 누락된 레이블을 생성할 수 있습니다.
