...
.시작하기 전에
워크로드 팩토리가 AWS 계정에서 Amazon EBS(Elastic Block Store), EFS(Elastic File System) 및 FSx for Windows File Server 스토리지 환경을 감지하려면 link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["grant_read_permissions 를 선택합니다"^]AWS 계정에 있어야 합니다.
