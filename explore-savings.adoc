...
.시작하기 전에
Workload Factory가 AWS 계정에서 Amazon Elastic Block Store(EBS), Elastic File System(EFS) 및 FSx for Windows File Server 스토리지 환경을 감지하려면 다음을 확인하세요.link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["_읽기 전용_ 권한 부여"^] 귀하의 AWS 계정에서.
