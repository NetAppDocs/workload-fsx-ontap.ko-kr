...
summary: Workload Factory에서 볼륨 크기를 수동으로 조정하고 볼륨 자동 확장을 사용하는 등 볼륨 용량을 관리하는 방법을 알아보십시오. 
---
= NetApp Workload Factory에서 볼륨 용량을 관리합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
볼륨 용량은 필요에 따라 조정해야 합니다. 볼륨 크기를 수동으로 조정하거나 볼륨 자동 확장을 켜서 Workload Factory가 볼륨 용량을 관리하도록 할 수 있습니다. FlexVol 또는 FlexGroup 볼륨 용량의 균형을 확인하고, 노드 내의 모든 FlexVol 볼륨 또는 모든 구성 요소에 파일이 고르게 분산되도록 볼륨 용량을 재조정하여 모든 노드가 단일 FlexGroup 볼륨의 워크로드에 참여하도록 하십시오.
...
Workload Factory가 볼륨 용량을 자동으로 관리하도록 하려면 볼륨 자동 확장 기능을 켜십시오. 이 기능은 언제든지 끌 수 있습니다.


NOTE: iSCSI 볼륨에는 볼륨 자동 확장이 지원되지 않습니다.

.시작하기 전에
볼륨 자동 확장을 활성화하려면 링크를 연결해야 합니다. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["기존 링크를 연결하는 방법이나 새 링크를 만들고 연결하는 방법을 알아보세요."^]. 링크가 연결되면 이 작업으로 돌아오십시오.
...
.시작하기 전에
볼륨 용량을 조정하려면 링크를 연결해야 합니다. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["기존 링크를 연결하는 방법이나 새 링크를 만들고 연결하는 방법을 알아보세요."^]. 링크가 연결되면 이 작업으로 돌아오십시오.
...
== FlexVol 및 FlexGroup 볼륨에 대한 볼륨 용량 재조정
...
.FlexVol 볼륨
FlexVol 볼륨은 데이터 관리에 유연성을 제공하는 논리적 컨테이너로, 확장, 축소, 이동 및 효율적인 복사를 지원합니다. NAS 및 SAN 환경에서 사용할 수 있습니다.
...
.FlexGroup 볼륨
반면 FlexGroup 볼륨은 고성능 및 자동 로드 분산을 위해 설계된 확장 가능한 NAS 컨테이너입니다. 트래픽을 투명하게 공유하는 여러 멤버 볼륨(구성 요소)으로 구성됩니다. FlexGroup 볼륨은 최대 60PB의 용량과 4,000억 개의 파일을 지원하여 FlexVol 한도를 초과하는 대용량을 제공합니다. 단일 네임스페이스 컨테이너를 제공하여 관리를 간소화합니다.
...
FlexGroup 볼륨 용량의 균형을 확인하는 작업에는 구성 요소의 현재 레이아웃을 평가하는 작업이 포함됩니다.  볼륨의 용량을 재조정할 때 NetApp Workload Factory는 모든 HA 쌍에 데이터를 균등하게 분산하기 위해 짝수 개의 구성 요소를 포함하는 새로운 구성 요소 레이아웃을 설계합니다.  이 서비스는 재조정 계획을 실행하여 읽기 및 쓰기 작업을 개선합니다.
...
* 볼륨의 균형을 확인하려면 link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["파일 시스템에 대한 링크를 생성하고 연결하거나 기존 링크를 연결합니다."^]해야 합니다. 링크가 연결되면 이 작업으로 돌아오십시오.
...
. *FSx for ONTAP*에서 재조정할 볼륨이 포함된 파일 시스템의 작업 메뉴를 선택한 다음 *관리*를 선택합니다.
...
NOTE: Workload Factory 관리자는 다음을 수행할 수 있습니다.<<볼륨 재조정 작업 중지,리밸런싱을 중지하다>> 수술 중.

.시작하기 전에
* <<볼륨의 균형을 확인하세요,볼륨의 균형을 확인하세요>> 볼륨을 재조정하기 전에,
* 볼륨 균형을 재조정하려면 link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["파일 시스템에 대한 링크를 생성하고 연결하거나 기존 링크를 연결합니다."^]해야 합니다. 링크가 연결되면 이 작업으로 돌아가십시오.
