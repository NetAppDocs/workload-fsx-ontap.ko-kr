...
---
= NetApp Workload Factory에서 복제 관계 생성
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
NetApp Workload Factory에서 FSx for ONTAP 파일 시스템에 대한 복제 관계를 생성하여 예기치 못한 재해 발생 시 데이터 손실을 방지하십시오. 복제는 두 FSx for ONTAP 파일 시스템 간, 그리고 Cloud Volumes ONTAP 또는 온프레미스 ONTAP 시스템과 FSx for ONTAP 파일 시스템 간에 지원됩니다.

.이 작업에 대해
복제는 데이터가 저장된 지역에서 재해가 발생할 경우 데이터 손실을 방지하는 보호 기능을 제공합니다.

이 작업은 FSx for ONTAP 파일 시스템, 온프레미스 ONTAP 시스템 또는 Cloud Volumes ONTAP 시스템의 소스 볼륨에 대한 복제 관계를 생성합니다.

타겟 파일 시스템의 복제된 볼륨은 데이터 보호(DP) 볼륨이며 다음과 같은 명명 형식을 `{OriginalVolumeName}_copy` 따릅니다.

불변 파일이 포함된 소스 볼륨을 복제하면 소스 볼륨의 불변 파일 보존 기간이 만료될 때까지 타겟 볼륨과 파일 시스템이 잠긴 상태로 유지됩니다. 불변 파일 기능은 FSx for ONTAP 파일 시스템에 대해 link:create-volume.html["볼륨을 생성하다"]할 때 사용할 수 있습니다.
...
파일 시스템의 특정 볼륨 또는 모든 볼륨을 복제하려면 다음 단계를 따르십시오.
