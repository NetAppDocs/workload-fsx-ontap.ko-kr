...
summary: 이 기능을 활성화하면 Workload Factory가 시간이 지남에 따라 용량 요구 사항이 변경됨에 따라 FSx for ONTAP 파일 시스템에 증분 스토리지를 자동으로 추가할 수 있습니다. 
---
...
[role="lead"]
자동 용량 관리를 활성화하면 NetApp Workload Factory가 시간이 지남에 따라 용량 요구 사항이 변경됨에 따라 FSx for ONTAP 파일 시스템에 증분 스토리지를 자동으로 추가할 수 있습니다.  또한, 이 기능을 활성화하면 수동으로 용량을 모니터링할 필요가 없고, 영향을 미치는 애플리케이션 데이터 읽기 및 쓰기의 위험이 완화됩니다.
...
* 이 작업을 완료하려면 Workload Factory에서 _읽기/쓰기_ 권한이 있는 자격 증명이 있어야 합니다.
